...
ONNX_OPERATOR_SCHEMA(Cast)
    .SetDoc(R"DOC(
Casts each element of the input tensor to a data type
specified by the 'to' attribute and returns the converted elements in the output.
NOTE: Casting to and from strings is not supported yet.
)DOC")
    .Attr(
        "to",
        "The data type to which the elements of the input tensor are cast."
        " One of 'UINT8,' 'INT8,' 'UINT16,' 'INT16,' 'INT32,' 'UINT32,' 'INT64,' 'UINT64,' 'BOOL,'FLOAT,' 'FLOAT16,' or 'DOUBLE'",
        AttributeProto::STRING)
    .Input(0, "input", "Input tensor to be cast.", "T1")
    .Output(
        0,
        "output",
        "Output tensor with the same shape as input. Its element type "
        "is specified by the 'to' attribute.",
        "T2")
    .TypeConstraint(
        "T1",
        {"tensor(float16)",
...
ONNX_OPERATOR_SCHEMA(Shape)
    .SetDoc(R"DOC(
Takes a tensor as input and outputs an 1-dimensional tensor representing the shape of the input tensor.
)DOC")
    .Input(0, "data", "An input tensor.", "T")
...
ONNX_OPERATOR_SCHEMA(Size)
    .SetDoc(R"DOC(
Takes a tensor as input and outputs one with a single integer equal to the total number of elements of the input tensor.
)DOC")
    .Input(0, "data", "An input tensor.", "T")
...
ONNX_OPERATOR_SCHEMA(Concat)
    .SinceVersion(4)
    .Attr("axis", "The axis to concat along.", AttributeProto::INT)
    .SetDoc("Concatenate a list of tensors into a single tensor of the same element type.")
    .Input(
        0,
        "inputs",
...
ONNX_OPERATOR_SCHEMA(Split)
    ...
    .Attr("axis", "The axis to split along.", AttributeProto::INT)
    .Attr("split", "The length of each output section", AttributeProto::INTS, OPTIONAL)
    .SetDoc(R"DOC(Split a tensor into a list of tensors, along the specified
'axis'. The lengths of each individual part can be specified using the attribute 'split'.
If 'split' is absent, the tensor is split into equal-sized parts.<br>
**TODO: What if there are too many output parts? Does the operator fail or leave some outputs empty?**
)DOC");
...
ONNX_OPERATOR_SCHEMA(Transpose)
    .SetDoc(R"DOC(
Transpose the input tensor similar to numpy.transpose.<br><br>For example, given
``perm=[1, 0, 2]`` and an input tensor of shape ``[1, 2, 3]``, the output shape
will be ``[2, 1, 3]``.
)DOC")
    .Attr(
        "perm",
        "An list of integers specifying the order in which to permute the dimensions."
        " By default, Transpose will reverse the dimensions.",
        AttributeProto::INTS,
        OPTIONAL)
    .Input(0, "data", "An input tensor.", "T")
...
Takes an attribute `axes` with a list of axes to squeeze.
)DOC")
...
Takes one required attribute `axes`, which is a list of dimensions that will be inserted.
...
ONNX_OPERATOR_SCHEMA(Pad)
    ...
    .Attr(
        "mode",
        "Three modes: 'constant,' 'reflect,' and 'edge.' The default is 'constant.'",
        AttributeProto::STRING,
        std::string("constant"))
    .Attr(
        "value",
